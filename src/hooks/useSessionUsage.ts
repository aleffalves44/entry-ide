...
function totalOf(r: FrameworkUsageEntry): number {
...
export function deriveAgentLines(rows: FrameworkUsageEntry[]): SessionAgentLine[] {
  const main: SessionAgentLine = { agent: "main", runs: 0, outputTokens: 0, totalTokens: 0, costUsd: 0 };
  const subs = new Map<string, SessionAgentLine>();
  for (const r of rows) {
    if (r.kind === "turn") {
      main.runs += 1;
      main.outputTokens += r.output_tokens;
      main.totalTokens += totalOf(r);
      main.costUsd += r.cost_usd;
    } else if (r.kind === "agent") {
      let line = subs.get(r.agent);
      if (!line) {
        line = { agent: r.agent, runs: 0, outputTokens: 0, totalTokens: 0, costUsd: 0 };
        subs.set(r.agent, line);
      }
      line.runs += 1;
      line.outputTokens += r.output_tokens;
      line.totalTokens += totalOf(r);
      line.costUsd += r.cost_usd;
    }
  }
  const sorted = [...subs.values()].sort((a, b) => b.totalTokens - a.totalTokens);
